...
ai-eval bootstrap -- pytest -q     # capture real runtime traces
ai-eval run                        # score the suite against the golden set
ai-eval diff --against last        # show metric regressions + narratives
```
...
4. **Run as normal** — `ai-eval init`, `ai-eval analyze`, etc. use the local judge
   automatically.
...
```bash
ai-eval init --rubric-engine rules
```
...
- `ai-eval doctor` — environment + config sanity check (incl. a judge-gateway probe;
...
- `ai-eval judge --list` — show configured judges (`default` / `regression_check` /
  `fallback[*]`) with a best-effort reachability ping.
- `ai-eval judge --ping <model>` — one-shot reachability probe of a single model id.
- `ai-eval judge --prompt <text> --model <m>` — one-shot judge completion.
- Async transport via `litellm.acompletion` + `instructor`; content-addressed
  cache under `.ai-eval/cache/judge/` (skipped with `--no-cache`). Two prompt
...
- `ai-eval bootstrap -- <command>` — captures real runtime traces (one golden
...
- `ai-eval run` — re-imports task modules, calls the entry symbol per golden
  example, scores each metric via the judge gateway, aggregates, and saves a
  run record (`.ai-eval/runs/<id>/run.json`) + history. `--fail-on-regression`
...
- `ai-eval diff --against last` — per-metric Δ table; `--explain` emits a
...
- `ai-eval report [last]` — render a past run as human / json / markdown
  (`--out <file>`); `--web` writes a minimal HTML file and prints a `file://` URL.
- `ai-eval history [--list|--show <id>|--prune --keep N|--export <file>]` —
  manage `.ai-eval/history.json` (slim records; full runs under `runs/<id>/`).
