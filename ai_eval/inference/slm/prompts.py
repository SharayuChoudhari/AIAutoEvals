...
def _load(name: str) -> str:
    ...
    return resources.files(_PROMPTS_PACKAGE).joinpath(f"{name}.txt").read_text(
        encoding="utf-8"
    )
...
def _few_shot_examples() -> list[dict]:
    ...
    raw = resources.files(_PROMPTS_PACKAGE).joinpath("few_shot.yaml").read_text(
        encoding="utf-8"
    )
...
def render(name: str, **fields: str) -> str:
    ...
    return _PLACEHOLDER_RE.sub(
        lambda m: str(fields.get(m.group(1), m.group(0))), template
    )
