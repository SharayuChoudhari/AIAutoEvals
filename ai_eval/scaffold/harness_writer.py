...
def _render_region1(spec: HarnessSpec) -> str:
    ...
    lines.append("        '_ai_eval_harness_target', _TASK_FILE)")

    lines.append("    if spec is None or spec.loader is None:")
    lines.append("        raise ImportError('cannot load ' + _TASK_FILE)")
    ...
    lines.append("    cls_name, _, _ = _TASK_ENTRY.rpartition('.')")
    lines.append("    cls = getattr(mod, cls_name, None) if cls_name else None")
    lines.append("    if cls is None:")
    lines.append("        return")
    for a in spec.attrs:
        lines.append(f"    _patch = _make_patch({a.attr!r}, {a.method!r})")
        lines.append(f"    setattr(cls, {a.method!r}, _patch)")
    lines.append("")
    lines.append("")
